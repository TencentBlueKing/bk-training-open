    ...
    <div class="myGroup">
        ...
        <div class="Group-Msg">
            ...
            <div class="Group-Msg-GroupName">
                ...
                <!-- 选择组 -->
                <bk-select :disabled="false" v-model="curGroupId" style="width: 250px;"
                    ext-cls="select-custom"
                    ext-popover-cls="select-popover-custom"
                    @selected="changeGroup"
                    searchable>
                    <bk-option v-for="option in AllgGroupsist"
                        :key="option.id"
                        :id="option.id"
                        :name="option.name">
                    </bk-option>
            ...
            <div class="Group-Msg-Admin">
                ...
                <div class="Group-Msg-Admin-List">
                    <div class="Group-Msg-Admin-Item" v-for="(item,index) in curGroupData.admin_list" :key="index">
                        {{item.username}}({{item.name}}) <span style="backgroundColor: white;" v-if="index !== curGroupData.admin_list.length - 1">&nbsp;/&nbsp;</span>
                    </div>
                </div>
            </div>
            <!-- 对组的功能 -->
            <div class="Group-Func">
                ...
                <div class="Group-Func-Major">
                    ...
                    <span> / </span>
                    <span @click="executeFunc('applyJoinGroup','请求入组')"> 请求入组</span>
                </div>
                <!-- 更多功能 -->
        ...
        <div class="Group-Member">
            ...
            <div class="Group-Member-Func">
                <div class="Group-Member-Func-add" @click="executeFunc('addGroupUser', '新增成员')">新增成员</div>
                <div class="Group-Member-Func-batchDel" @click="alldeleteUsers">批量删除</div>
            </div>
            <!-- 成员List -->
            <div>
                <bk-table style="margin-top: 15px;"
                    ...
                    <bk-table-column label="操作" width="150">
                        <template slot-scope="props">
                            <bk-button class="mr10" theme="primary" text @click="removeGroupUser(props.row)">移除</bk-button>
                        </template>
                    </bk-table-column>
                </bk-table>
            </div>
                ...
                <bk-form label-width="120">
                    ...
                    <bk-form-item label="管理员">
                        <bk-select
                            ...
                            <bk-option v-for="user in AllUsers"
                                :key="user.id"
                                :id="user.id"
                ...
                <bk-form label-width="60">
                    <bk-form-item label="组" required="true">
                        <bk-select
                            ...
                            <bk-option v-for="Group in NotJoinGroup"
                                :key="Group.id"
                                :id="Group.id"
                ...
                <bk-form label-width="120">
                    <bk-form-item label="新成员" required="true">
                        <bk-select
                            ...
                            <bk-option v-for="user in notakeUsers"
                                :key="user.id"
                                :id="user.id"
                                :name="user.username + '(' + user.display_name + ')'">
                            </bk-option>
...
<script>
    import { bkSelect, bkOption, bkDropdownMenu, bkDialog, bkTable, bkTableColumn, bkButton } from 'bk-magic-vue'
    import { getAlljurGroups, getGroupInfo, getAllUsers, getUser, addGroup, getNotJoinGroup, deleteGroup, getGroupUsers, addGroupUsers, deleteGroupUsers, updateGroup, ApplyJoinGroup } from '@/api/request.js'
    //
    export default {
        components: {
            bkSelect,
        ...
        computed: {
            dialogConfirm () {
                return this.funcName === 'addGroup' ? this.confirmAdd : this.funcName === 'applyJoinGroup' ? this.confirmApplyJoinGroup : this.funcName === 'compileGroup' ? this.compileGroup : this.funcName === 'addGroupUser' ? this.compileaddGroupUser : function () {
                }
            },
            dialogCancel () {
                return this.funcName === 'addGroup' ? this.cancelAdd : this.funcName === 'applyJoinGroup' ? this.cancelApplyJoinGroup : this.funcName === 'compileGroup' ? this.cancelcompileGroup : this.funcName === 'addGroupUser' ? this.canceladdGroupUser : function () {
                }
            }
        },
        created  () {
        ...
        methods: {
            ...
            confirmAdd () {
                ...
                addGroup({ name: this.newGroup, admin: this.selectAdminList }).then(res => {
                    this.$bkMessage({ offsetY: 80, message: '添加成功', theme: 'success'
                    })
            ...
            async confirmApplyJoinGroup () {
                ...
                Promise.all([...applyArr.map(item => {
                    ...
                    getNotJoinGroup().then(res => {
                        ...
                        this.isshowDialog = false
                        // 申请成功
                        this.$bkMessage({ offsetY: 80, message: '申请成功', theme: 'success'
                        })
                    })
                })
            },
            ...
            compileGroup () {
                ...
                updateGroup(this.curGroupId, { name: this.curGroupData.name, admin: this.selectCompileadminList }).then(res => {
            ...
            handlePageChange (newPage) {
                ...
                this.renderGroupUsers = this.groupUsers.slice((this.pagination.current - 1) * this.pagination.limit, this.pagination.current * this.pagination.limit)
            },
            // 新增成员
            compileaddGroupUser () {
                addGroupUsers(this.curGroupId, this.selectnotakeUsersID).then(res => {
                    if (res.result) {
                        ...
                        this.$bkMessage({ offsetY: 80, message: '新成员添加成功', theme: 'success'
                        })
                        this.changeGroup(this.curGroupId)
                    }
                })
                    ...
                    deleteGroupUsers(this.curGroupId, deleteForm).then(res => {
                        if (res.result) {
                            this.$bkInfo({
                                title: '确认要删除数据？',
                                confirmFn: () => {
                                    this.$bkMessage({ offsetY: 80, message: '移除成功', theme: 'success'
                                    })
                                    this.changeGroup(this.curGroupId)
                                }
                            })
            ...
            alldeleteUsers () {
                if (this.existAdmin()) {
                    this.$bkInfo({
                        title: '确认要批量删除数据？',
                        confirmFn: () => {
                            Promise.all(this.tableSelect.map(item => item.id).map(item2 => {
                                deleteGroupUsers(this.curGroupId, { 'user_id': item2 })
                            })).then(res => {
                                this.$bkMessage({ offsetY: 80, message: `成功移除${res.length}个成员`, theme: 'success'
                                })
                                this.changeGroup(this.curGroupId)
                            })
                        }
            ...
            existAdmin (id) {
                if (id) {
                    ...
                    if (this.curGroupData.admin_list.map(item => item.id).includes(id)) {
                        this.handleBox({ theme: 'error', message: '不能选择删除管理员', offsetY: 80 })
                        return false
                    }
                    return true
                } else {
                    // 群删
                    let flat = true
                    this.curGroupData.admin_list.map(item => item.id).forEach(id => {
