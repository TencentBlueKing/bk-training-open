<template>
    <div class="groupdaily">
        <div class="groupdaily-btn">
            <TabBtn :content="tabBtnContent" @changeType="selectedType" :active="curType"></Tabbtn>
            <!-- 成员选择器或者日期选择器 -->
            <div class="groupdaily-member-select" v-show="curType === 1">
                <bk-select
                    :disabled="false"
            ...
            <div class="groupdaily-date-select" v-show="curType === 2">
                <bk-date-picker font-size="medium" :options="customOption" @change="changeDate" style="width: 250px;" :clearable="false" behavior="simplicity" class="mr15" v-model="curDateTime"></bk-date-picker>
            </div>
        </div>
        <!-- 渲染的内容 -->
        <div class="renderlistbox" v-show="renderDaily && renderDaily.length">
            <bk-card class="all-report-card card"
                ...
                <div class="card-time">{{daily.date}}</div>
            ...
            <!-- 分页器 -->
            <div class="renderlistbox-pagination" v-show="curType === 1">
                <bk-pagination
                    size="small"
                    @change="changePage"
        ...
        <div class="notrender" v-show="renderDaily && renderDaily.length === 0">
            <div class="exception-wrap">
                <bk-exception ext-cls="notrender-box" class="exception-wrap-item exception-part" type="empty" scene="part" :class="{ 'exception-gray': isGray }"> 现在还没有日报 </bk-exception>
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    import { bkSelect, bkOption, bkDatePicker, bkException, bkPagination } from 'bk-magic-vue'
    ...
    export default {
        ...
        computed: {
            ...
            curselectUserId () {
                return this.$store.state.groupDaily.curUserID
            }
        },
        watch: {
            curselectUserId (oldVal) {
                this.curSelectUser = oldVal
            },
            curGroupIDChange (oldVal) {
                if (String(oldVal).slice(0, 1) === 'P') {
                    this.curGroupID = Number(String(oldVal).slice(1))
                    this.curType = 1
                    this.getAllGroupUsers(false)
                    this.getRenderDaily('', this.curSelectUser, this.pagingDevice.limit, this.pagingDevice.curPage).then(res => {
        ...
        activated () {
            if (this.curGroupIDChange !== null) {
                this.curGroupID = this.curGroupIDChange
                // 只要Vuex的组ID变动 组成员就重新获取
                this.getAllGroupUsers(true)
                // 如果用户组发生了变化开始
                this.curType = 2
